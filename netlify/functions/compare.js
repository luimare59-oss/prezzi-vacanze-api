...
// Airbnb: set "notte singola" (2–6) esclusivo
...
// Booking: “notte singola” = MARKUP esclusivo (tipicamente per 1 notte)
function bookingSingleNightApplicable({nights, checkin, env}) {
  ...
  return monthOk && nights === 1; // adatta qui se vuoi altre durate
}
...
// Una riga per OTA con tutte le regole
function computeRow({ ota, checkin, nights, guests, env }) {
  ...
  let disc = 0, offers = [], netNight = BASE;
  ...
  const fees = {
    "Airbnb":   { host: AIRBNB_HOST, guest: AIRBNB_GUEST },
    "Booking.com": { host: BKNG_HOST, guest: BKNG_GUEST },
    "Vrbo":     { host: VRBO_HOST, guest: VRBO_GUEST }
  }[ota];
  ...
  const guestFee  = +(preOta * fees.guest).toFixed(2);
  const guestTot  = +(preOta + guestFee).toFixed(2);
  const hostFee   = +(preOta * fees.host).toFixed(2);
  ...
  return {
    ...
    "Fee ospite %": fees.guest, "Fee ospite €": guestFee,
    "Totale ospite": guestTot,
    "Fee host %": fees.host, "Fee host €": hostFee,
    "Payout host": hostPay,
    Valuta: CURRENCY, Regime: regime
  };
}

// Genera righe per una singola data
function computeRowsForDate({ checkin, nights, guests, env }) {
  return [
    computeRow({ ota:"Airbnb",      checkin, nights, guests, env }),
    computeRow({ ota:"Booking.com", checkin, nights, guests, env }),
    computeRow({ ota:"Vrbo",        checkin, nights, guests, env }),
  ];
}

// Applica flag parità rispetto a baseline
function applyParityFlag(rows, baselineName, tol){
  const baseline = baselineName.toLowerCase();
  const baseRow = rows.find(r => r.OTA.toLowerCase() === baseline);
  if (!baseRow) return rows;
  const baseGuest = baseRow["Totale ospite"];
  const label = "Parità vs " + baseRow.OTA;
  rows.forEach(r => {
    const diff = (r["Totale ospite"] - baseGuest) / (baseGuest || 1);
    let flag = "OK";
    if (Math.abs(diff) > tol) flag = (diff > 0 ? `+${(diff*100).toFixed(1)}%` : `${(diff*100).toFixed(1)}%`);
    r[label] = flag;
  });
  return rows;
}

// CSV semplice (virgola)
function toCSV(rows){
...
// Genera lista di date (start..end inclusivo)
function genDatesBetween(startStr, endStr){
  const dates = [];
...
export const handler = async (event) => {
  ...
  // Parità
  const baseline = (q.baseline || "Booking.com").trim();
  const tol = num(q.tol, 0.02);

  // Multi-date
  let dateList = [];
  if (q.dates) {
    dateList = String(q.dates).split(",").map(s=>s.trim()).filter(Boolean);
  } else if (q.start && q.end) {
    dateList = genDatesBetween(q.start, q.end);
  } else {
    const checkin = q.checkin || new Date(Date.now()+7*24*60*60*1000).toISOString().slice(0,10);
    dateList = [checkin];
  }

  // Calcolo righe per tutte le date
  let allRows = [];
  for (const ci of dateList) {
    const rows = computeRowsForDate({
      checkin: ci,
      nights,
      guests,
      env: {
        ...
        // Booking: notte singola (markup) + stack sconti
    ...
    // Applica flag parità su questa data
    applyParityFlag(rows, baseline, tol);
  ...
  if (format === "csv") {
    return {
      ...
      headers: {
        ...
        "Content-Disposition": `inline; filename="compare_${dateList[0]}_${nights}n_${guests}g.csv"`
      },
      body: toCSV(allRows)
    };
  }
